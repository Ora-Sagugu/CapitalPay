<template>
  <div>
    <h1 class="ptitle">{{ $t('agent.remittanceTitle') }}</h1>
    <p class="psub">{{ $t('agent.remittanceSub') }}</p>
    <el-alert
    ...
    >
      {{ $t('agent.needOnboarding') }}
      <el-button link type="primary" @click="$router.push('/onboarding')">{{ $t('remittance.goOnboarding') }}</el-button>
    </el-alert>
    <el-alert
      v-else-if="!merchantsLoading && !eligibleMerchants.length"
      type="warning"
      :closable="false"
      style="margin-bottom: 16px"
    >
      {{ $t('agent.remittanceNoEligible') }}
    </el-alert>
    <el-row :gutter="16">
      <el-col :span="16">
        <div class="page-card">
          <el-form :model="form" label-width="110px" :disabled="!formEnabled">
            <div class="section-title">{{ $t('agent.remittanceSelectCustomer') }}</div>
            <el-form-item :label="$t('agent.remittanceSelectCustomer')" required>
              <el-select
                v-model="form.merchant_id"
                filterable
                clearable
                style="width: 100%"
                :placeholder="$t('agent.remittanceSelectPh')"
              ...
              >
                <el-option
                  ...
                  :key="m.id"
                  :label="`${m.merchant_name} (${m.merchant_no})`"
                  :value="m.id"
                  :disabled="!m.remittance_eligibility?.eligible"
                />
              </el-select>
            </el-form-item>
            <el-alert
              v-if="selectedIneligibleMessage"
              type="warning"
              :closable="false"
              style="margin-bottom: 16px"
              :title="selectedIneligibleMessage"
            />
...
const eligibleMerchants = computed(() =>
  merchants.value.filter((m) => m.remittance_eligibility?.eligible)
)

const selectedMerchant = computed(() =>
  merchants.value.find((m) => m.id === form.merchant_id) || null
)

const formEnabled = computed(() => auth.onboardingStatus === 'approved')

const canSubmit = computed(() =>
  formEnabled.value && !!selectedMerchant.value?.remittance_eligibility?.eligible
)

const selectedIneligibleMessage = computed(() => {
  const merchant = selectedMerchant.value
  if (!merchant || merchant.remittance_eligibility?.eligible) return ''
  const blocker = merchant.remittance_eligibility?.blockers?.[0]
  if (!blocker) return t('agent.remittanceIneligible')
  const key = `remittance.blockers.${blocker.code}`
  return te(key) ? t(key) : (blocker.message || t('agent.remittanceIneligible'))
})
...
async function runSanctionCheck() {
  ...
  try {
    const result = await agentSanctionCheck({
...
async function preview() {
  ...
  if (!canSubmit.value || !form.merchant_id || !Number.isFinite(numericAmount) || numericAmount <= 0) {
    invalidateQuote()
    return
  }
  ...
  // #region agent log
  fetch('http://127.0.0.1:7400/ingest/4ebbb499-0df2-4dae-83ef-65d3a7131853',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'cc4e4a'},body:JSON.stringify({sessionId:'cc4e4a',runId:'quote-check',hypothesisId:'H-A',location:'AgentRemittance.vue:preview',message:'agent quote request',data:{requestNo,merchant_id:form.merchant_id,amount:form.amount,from:form.from_currency,to:form.to_currency,fee_bearing:form.fee_bearing},timestamp:Date.now()})}).catch(()=>{});
  // #endregion
  try {
    const quote = await agentRemittanceQuote({
    ...
    if (requestNo === quoteRequest) {
      fee.value = quote
      // #region agent log
      fetch('http://127.0.0.1:7400/ingest/4ebbb499-0df2-4dae-83ef-65d3a7131853',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'cc4e4a'},body:JSON.stringify({sessionId:'cc4e4a',runId:'quote-check',hypothesisId:'H-C',location:'AgentRemittance.vue:preview:response',message:'agent quote applied to panel',data:{requestNo,fee_bearing:quote?.fee_bearing,total_fee:quote?.total_fee,sender_fee:quote?.sender_fee,beneficiary_fee:quote?.beneficiary_fee,sender_total:quote?.sender_total,settle_amount:quote?.settle_amount,exchange_rate:quote?.exchange_rate,from:quote?.from_currency,to:quote?.to_currency},timestamp:Date.now()})}).catch(()=>{});
      // #endregion
    }
  } catch (err) {
    // #region agent log
    fetch('http://127.0.0.1:7400/ingest/4ebbb499-0df2-4dae-83ef-65d3a7131853',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'cc4e4a'},body:JSON.stringify({sessionId:'cc4e4a',runId:'quote-check',hypothesisId:'H-A',location:'AgentRemittance.vue:preview:error',message:'agent quote failed',data:{requestNo,fee_bearing:form.fee_bearing,error:String(err?.message||err)},timestamp:Date.now()})}).catch(()=>{});
    // #endregion
  }
  finally {
...
function validateForm() {
  if (!form.merchant_id) {
    ElMessage.warning(t('agent.remittanceCustomerRequired'))
    return false
  }
...
async function submit() {
  ...
  if (!canSubmit.value) {
    ElMessage.warning(selectedIneligibleMessage.value || t('agent.remittanceIneligible'))
    return
  }
  ...
  try {
    if (!idempotencyKey) {
      idempotencyKey = globalThis.crypto?.randomUUID?.()
        || `rmt-ag-${Date.now()}-${Math.random().toString(16).slice(2)}`
    }
    await agentRemittanceApply({
      merchant_id: form.merchant_id,
    ...
    ElMessage.success(t('agent.remittanceSubmitted'))
...
async function loadMerchants() {
  ...
  try {
    ...
    merchants.value = data.items || data.results || []
    const preferred = String(route.query.merchant_id || '')
    const eligible = merchants.value.filter((m) => m.remittance_eligibility?.eligible)
    if (preferred && eligible.some((m) => m.id === preferred)) {
      form.merchant_id = preferred
    } else if (!form.merchant_id && eligible.length === 1) {
      form.merchant_id = eligible[0].id
    }
  } finally {
